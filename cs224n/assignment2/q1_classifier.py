...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        # tf.Variable is the parameter (that we want to train on) here it's W and b
        W = tf.Variable(tf.random_normal([self.config.n_features, self.config.n_classes],
                                         stddev=1.0/tf.sqrt(tf.to_float(self.config.n_classes))),
                        name='W')
