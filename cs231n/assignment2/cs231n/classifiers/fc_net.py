...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        b2 = self.params['b2']
        # affine, relu
        h_out, h_out_cache = affine_relu_forward(X, W1, b1)
        # affine, softmax
        ...
        dh_out, dW2, db2 = affine_backward(dscores, scores_cache)
        _, dW1, db1 = affine_relu_backward(dh_out, h_out_cache)
        dW1 += self.reg * W1
        dW2 += self.reg * W2
        grads['W1'] = dW1
        grads['W2'] = dW2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, len(hidden_dims) + 1):
            ...
            if i == 1:
                self.params[wi] = weight_scale * np.random.randn(input_dim, hidden_dims[i-1])
            else:
                self.params[wi] = weight_scale * np.random.randn(hidden_dims[i-2], hidden_dims[i-1])

            if i == len(hidden_dims):
                wlast = param_w + str(i+1)
                blast = param_b + str(i+1)
                self.params[wlast] = weight_scale * np.random.randn(hidden_dims[i-1], num_classes)
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        # note the architecture below
        # {affine - [batch norm] - relu - [dropout]}x(num_layers - 1) - affine - softmax
        param_w = 'W'
        param_b = 'b'
        ca = 'ca'  # cache affine
        cr = 'cr'  # cache relu
        ...
        loss, dscores = softmax_loss(scores, y)
        # the last layer
        wlast = param_w + str(self.num_layers)
        blast = param_b + str(self.num_layers)
        d_aff_out, grads[wlast], grads[blast] = affine_backward(dscores,
                                                    caches[ca + str(self.num_layers)])
        grads[wlast] += self.reg * self.params[wlast]
        loss += 0.5 * self.reg * np.sum(self.params[wlast] ** 2)
